...
A LSTM network is a gated RNN that overcomes two key drawbacks of a crude RNN.
\begin{enumerate}
...
Figure~\ref{fig:QQ_GMWB} and Figure~\ref{fig:QQ_NN} shows the QQ plots between the \textbf{true} labels and the predicted losses for the regression metamodels and the neural network metamodels, respectively.
...
Figure~\ref{fig:tailMatches} depicts the average percentage\footnote{The average percentage is calculated based on 50 macro replications.} of correctly identified true tail scenarios by different metamodels for different safety margins.
...
\begin{table}[ht!]
    ...
    \begin{tabular}{lccccc}
        ...
        \midrule
        Low-capcity   LSTM & Low    & $0.082 (\pm0.006)$ & $0.070 (\pm0.006)$ \\ 
...
The QQ-plots depicted in Figure~\ref{fig:QQ_All} illustrates the fit of the two LSTM metamodels when different noise levels are present in the training labels.
They are arranged in a 2-by-3 grid, where to the top represents an increase in model capacity, and shifting to the right corresponds to an increase in noise levels within the training labels. 
The sparsity of the plot increases as we move from the bottom-left to the top-right corner.
...
In addition to the percentage of correctly identified true tail scenarios, Table~\ref{tab:lstm_corr} reports the Spearman rank correlation coefficients that measure the metamodels' ability to rank the scenarios by their true losses.
The Pearson correlation coefficients are also included to illustrate the linear correlation between the predicted losses and the true losses.
The p-values of these quantities are less than $10^{-5}$, indicating that the correlations are statistically significant.
...
\begin{table}[ht!]
    ...
    \begin{tabular}{lccccc}
        ...
        \bottomrule
    \end{tabular}
    \caption{Spearman and Pearson correlation coefficients between LSTM predicted losses and true losses.}
    \label{tab:lstm_corr}
\end{table}

The effect of training label noise is more pronounced for the high-capacity LSTM than for the low-capacity LSTM.
...
\begin{table}[ht!]
    ...
    \begin{tabular}{lccccc}
        ...
        \bottomrule
    \end{tabular}
    \caption{Sensitivity of low-capacity LSTM metamodel to data size and noise level.}
    \label{tab:lstm_sens}
\end{table}

Table~\ref{tab:lstm_sens} shows the average squared errors and the half widths of their 95\% confidence intervals between the predictions and the true losses for various combinations of $M$ and $N$.
We observe that a decrease in data size has a profound impact on the widths of the confidence intervals.
